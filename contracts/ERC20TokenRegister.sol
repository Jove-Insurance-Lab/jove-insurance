...
contract ERC20TokenRegister is JoveRegister
{
    using SafeMath for uint256;
    using CompatibleERC20 for address;

    constructor() public
        JoveRegister(msg.sender)
    {
    }
    ...
    function getTokenBalance(bytes8 coinName, address addr) view public returns(uint256)  {
        require(hasContract(coinName), "coinName is not registed");
	address coinAddress=getToken(coinName);
	uint256 coinBalance=coinAddress.balanceOfERC20(addr);   	
	return coinBalance;
    }
